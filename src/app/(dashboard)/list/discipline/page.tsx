...
import { Class, DisciplinaryHistory, Prisma, Student } from "@prisma/client";

import Image from "next/image";
import Link from "next/link";
import FormContainer from "@/components/FormContainer";

type DisciplinaryHistoryList = DisciplinaryHistory & { class: Class } & {
  student: Student;
};
...
const DisciplinaryHistoryPage = async ({
  searchParams,
}: {
  ...
  const renderRow = (item: DisciplinaryHistoryList) => (
    <tr
      key={item.id}
    ...
    >
      <td className="flex items-center gap-4 p-4">
        ...
        <div className="flex flex-col">
          <h3 className="font-semibold">{`${item.student?.firstName} ${item.student?.lastName}`}</h3>
  ...
  const query: Prisma.DisciplinaryHistoryWhereInput = {};
  ...
  const [data, count] = await prisma.$transaction([
    prisma.disciplinaryHistory.findMany({
      where: query,
      include: {
        class: true,
        student: true,
    ...
    prisma.disciplinaryHistory.count({ where: query }),
  ]);
...
export default DisciplinaryHistoryPage;
